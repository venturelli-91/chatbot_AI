...
Conversational chatbot powered by a local [Ollama](https://ollama.ai/) instance. Tokens stream in real time, the model remembers previous turns, and the whole UI is fully accessible — dark and light modes included.
...
| Area                     | Details                                                                          |
| ------------------------ | -------------------------------------------------------------------------------- |
| **Streaming**            | Tokens appear as generated — animated cursor while loading                       |
...
| **Session history**      | Past conversations persisted to `localStorage`                                   |
| **Theme**                | Dark "Nebula" + light mode toggle                                                |
| **Security**             | SSRF prevention, rate limiting (20 req/min/IP), Zod validation, security headers |
...
**Prerequisites:** Node.js 20+ and [Ollama](https://ollama.ai/) running on port `11434`.
...
```bash
git clone https://github.com/venturelli-91/chatbot_AI.git
cd chatbot_AI
npm install
npm run dev        # http://localhost:3000
```

To use a different Ollama URL, set `OLLAMA_URL` in `.env.local`.
